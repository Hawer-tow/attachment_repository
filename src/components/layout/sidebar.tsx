...
import { BarChart2, BookOpen, Building2, CalendarDays, ChevronLeft, ChevronRight, Coffee, ConciergeBell, History, LayoutDashboard, LogOut, Mail, Settings as SettingsIcon, Sparkles, Tag, Users } from 'lucide-react';
      ...
      {user && (
        <div className="border-t border-white/10 p-3">
          <div className={cn('flex items-center gap-3 rounded-xl bg-white/8 p-2', !sidebarOpen && 'justify-center')}>
            ...
            {sidebarOpen && <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-white">{user.name}</p><p className="truncate text-xs capitalize text-cyan-100/60">{role}</p></div>}
          </div>
          {sidebarOpen && (
            <p className="mt-2 px-2 text-[10px] uppercase tracking-wide text-cyan-100/40">
              Access: {role === 'admin' ? 'All pages' : `${(ROLE_PAGES[role ?? '']?.filter(p => p !== '/' && p !== '/about' && p !== '/contact').length ?? 0)} pages`}
            </p>
          )}
